...
/// @title BnkrTreeEscrowV2 — Drip-feed vault for large USDC deposits into BNKR Tree Funding Vault
...
/// @dev v2: per-drip share accounting, exact approvals, bounded rescue, reentrancy guard,
///        strict cancel/claim semantics. Fixes all 5 issues from Claude's v1 review.
///
/// v1 → v2 changelog:
...
contract BnkrTreeEscrowV2 {
    ...
    function drip(uint256 dripId) external onlyKeeper nonReentrant {
        ...
        try VAULT.deposit(chunk) {
            ...
            if (d.retryCount >= 2) {
                ...
                // depositor can cancelDrip to retrieve it
    ...
    function claimShares(uint256 dripId) external nonReentrant {
        ...
        if (sharesToReturn == 0) revert NoSharesToClaim();
        ...
        d.sharesClaimed += sharesToReturn; // v2: mark claimed — idempotent

        if (usdcOut > 0) {
            USDC.transfer(d.depositor, usdcOut);
        }

        emit SharesClaimed(dripId, d.depositor, sharesToReturn, usdcOut);
    }
